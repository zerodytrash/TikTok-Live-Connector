  ...
  async (
    ...
    try {
      ...
      throw new SignAPIError(
        ErrorReason.CONNECT_ERROR,
        undefined,
        undefined,
        'Failed to connect to sign server.', err
      );
    }
    ...
    if (!response.headers['x-set-tt-cookie']) {
      const extra = response.headers?.['handshake-msg'] ? `Handshake Message: ${response.headers?.['handshake-msg']}` : '';
