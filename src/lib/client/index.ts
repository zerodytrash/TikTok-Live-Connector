...
import { createWebSocketProvider, WebcastWebSocketClient } from '@/lib/ws/lib/ws-client';
import {
  DecodedData,
...
export class TikTokLiveConnection extends (EventEmitter as WebcastTypedClient) {
  ...
  protected async _connect(roomId?: string): Promise<void> {
    ...
    if (this.options.useMobile && !this.options.authenticateWs) {
      throw new TypeError('Mobile WebSocket requires authenticateWs=true!');
    }

    if (this.options.authenticateWs) {

      const cookieBundle = await this.webClient.cookieJar.getSessionBundle();
      const oauthBundle = this.webClient.oAuthSessionBundle;

      if (!cookieBundle && !oauthBundle) {
        throw new TypeError('A cookie or OAuth bundle is required to use the authenticated WebSocket!');
      }
  ...
  protected async setupWebsocket(
    ...
    const wsClient = this._wsClientInstance = this._wsClientProvider(
      {
        ...
        wsParams: wsParams,
        wsHeaders: {
          Cookie: this.options.authenticateWs ?
            await this.webClient.cookieJar.getCookieString() :
            await this.webClient.cookieJar.getAnonymousCookieString()
        }
      }
    );
    ...
    const connectPromise = new Promise<void>((resolve, reject) => {
      ...
      const onUnexpectedResponse = (_, res) => {
        clearTimeout(connectTimeout);
        reject(new Error(`Error connecting to WebSocket with message: ${res.headers?.['handshake-msg'] || 'N/A'}`));
      };
      ...
      const onConnectOpen = () => {
        ...
        wsClient.removeListener('unexpected-response', onUnexpectedResponse);
      ...
      wsClient.once('unexpected-response', onUnexpectedResponse);
